...
**취소 대사 (김반장 확정 2026-09-07)**

1. 취소 포스트백의 `payment`는 **처리 로직에서 쓰지 않는다**. 그 값은 본상품 가격이라 회원이 실제 낸 돈과 다르다
...
> 위 1~4는 **처리 로직의 규칙**이다. CMS 「포스트백 로그」 화면은 수신 확인 전용이라
> 이 계산을 하지 않는다 — 받은 값을 받은 그대로 보여 주고, 같은 `orderId` + 같은 상품번호로
> 구매·취소 포스트백을 **연결해 보여 주는 것까지만** 한다(금액 계산 없음).

### CMS 「포스트백 로그」 화면 — 컬럼 ↔ 수신 필드 매핑

이 화면은 **수신 확인 전용**이다(김반장 확정 2026-09-07). 모든 컬럼이 수신 필드와 1:1로 대응하며,
계산해서 만든 값(제휴 수수료 매출·환수 금액·상태 판정·적립 예정 등)은 넣지 않는다.

**쿠팡 블록**

| 컬럼 | 수신 필드 |
|---|---|
| 구매 시각 | `purchase_time` (원문 병기) |
| 구분 | `purchase_cancel` |
| 회원 | `subid` |
| 주문번호 | `orderId` |
| OS | `os` |
| 상품 (펼침) | `order_detail[]` |
| └ 상품번호 | `order_detail[].productid` (구매) / `.productId` (취소) — 어느 키로 왔는지 함께 표기 |
| └ productName | `order_detail[].productName` |
| └ payment | `order_detail[].payment` |
| └ quantity | `order_detail[].quantity` |
| └ 대사 | 같은 `orderId` + 같은 상품번호의 반대편 포스트백 존재 여부 (연결만) |
| 펼침 원문 | `afcode`, `adid`, `subparam` |

**링크프라이스 블록** — 포스트백 1건 = 1행 (묶지 않음)

| 컬럼 | 수신 필드 |
|---|---|
| 주문 일시 | `day` · `time` (원문 병기) |
| 머천트 | `merchant_id` |
| 회원 | `affiliate_user_id` |
| 주문코드 | `order_code` |
| 상품 | `product_code` · `product_name` |
| 수량 | `item_count` |
| 구매금액 | `price` |
| 수수료 | `commision` — 링크프라이스가 금액을 직접 보내 주는 값이라 그대로 표시 |
| 펼침 원문 | `uniq_id`, `trlog_id`, `category_code`, `base_commission`, `incentive_commission`, `affiliate_id` |

검색·필터는 로그를 찾기 위한 것이라 둔다(쿠팡: `purchase_cancel` 필터 / 링크프라이스: `merchant_id` 필터).
정렬은 수신 시각 내림차순이며, 값을 바꾸지 않는다.
...
- 한 주문에 상품이 여럿이면 **상품 수만큼 포스트백이 따로 온다**. `uniq_id`가 같은 건을 한 주문으로
  ...
  이 묶음도 **처리 로직 쪽 규칙**이다. 로그 화면은 온 그대로 한 건 한 줄로 나열한다.
